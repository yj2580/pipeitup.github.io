...
hist(log(suicide$suicides_100k_pop))
...
maindata$log_suicide[is.infinite(maindata$log_suicide)]=0.01
model1=lm(log_suicide~log(gdp_for_year)+year+sex+age_group+gdp_for_year+gdp_per_capita, data = maindata)
...
```{r}
datatry= maindata %>% select(sex, age_group, suicides_no, log_suicide, gdp_for_year, gdp_per_capita, population)
...
```{r}
us=maindata %>% filter(country=="United States")
hist(us$log_suicide)
...
```{r}
library(splines2)
fitSSpline <- smooth.spline( x = maindata$gdp_per_capita, y = maindata$log_suicide, cv = TRUE)
```
